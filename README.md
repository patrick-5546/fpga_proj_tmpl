...
`verible-verilog-format`, and `surfer` are available on `PATH`. Python tools are
installed and invoked through uv. The local Python version is pinned to `3.13`
in `.python-version` because the current cocotb release does not support Python
3.14.
